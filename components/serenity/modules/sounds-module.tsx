  ...
  return (
    <section className="space-y-6 animate-in fade-in duration-300">
      ...
      >
        {currentSound ? (
          <>
            {/* YouTube Embed WITH AUDIO - The video provides the actual ambient sound */}
            <iframe
              key={`${currentSound.videoId}-${isMuted}`}
              src={`https://www.youtube.com/embed/${currentSound.videoId}?autoplay=1&mute=${isMuted ? 1 : 0}&loop=1&playlist=${currentSound.videoId}&controls=0&showinfo=0&rel=0&modestbranding=1&playsinline=1&enablejsapi=1`}
              className="absolute inset-0 w-full h-full"
              allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture"
              allowFullScreen
              title={`${currentSound.name} video`}
            />
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
        {filteredSounds.map((sound) => (
          <button
            ...
            className={`group relative aspect-[4/3] rounded-2xl overflow-hidden shadow-lg transition-all duration-300 hover:shadow-xl hover:scale-[1.02] ${
              isPlaying(sound.id) ? "ring-4 ring-primary ring-offset-2 ring-offset-background" : ""
            }`}
          >
            {/* Image Background */}
            ...
            <div className={`absolute inset-0 flex items-center justify-center transition-all duration-300 ${
              isPlaying(sound.id) ? "opacity-100" : "opacity-0 group-hover:opacity-100"
            }`}>
              <div className={`w-14 h-14 rounded-full flex items-center justify-center backdrop-blur-sm transition-all ${
                isPlaying(sound.id) 
